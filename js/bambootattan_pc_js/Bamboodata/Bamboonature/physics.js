...
$(function(){
    ...
    querySpecPageUrl = baseUrl+'/spec/findAllQueryWithOutPhysics';
    inti_page();
    //新增点击事件
    ...
    $('#exampleModal').on('shown.bs.modal',function () {
        $('#physicalForm').data('bootstrapValidator').resetForm();
    });
    //批量删除点击事件
    ...
    checkForm();
    //表单验证
    $('#physicalForm').bootstrapValidator();
    //初始化表格
    init_table();
    init_spec_table();
    // //表单验证
...
//根据权限初始化页面
function  inti_page() {
    if(hasAuthority('physical','auth_create')){
        $('#btn_add').removeClass('hide');
    }else{
        $('#btn_add').addClass('hide');
    }
    if(hasAuthority('physical','auth_delete')){
        ...
        columns:[//列数据
            ...
            },
            {
                ...
                formatter:function(value,row,index){//格式化，自定义内容
                    var _html='';
                    if(hasAuthority('physical','auth_edit')){
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                ...
                var validateForm = $('#physicalForm').data('bootstrapValidator');
                //手动触发验证
