...
$(function(){
    queryPageUrl = baseUrl+'/structure/findAllQuery';
    //querySpecPageUrl = baseUrl+'/spec/findAllQuery';
    querySpecPageUrl = baseUrl+'/spec/findAllQueryWithOutStructure';
    inti_page();
    //新增点击事件
    ...
    $('#exampleModal').on('shown.bs.modal',function () {
        $('#structureForm').data('bootstrapValidator').resetForm();
    });
    //批量删除点击事件
    ...
    checkForm();
    //表单验证
    $('#structureForm').bootstrapValidator();
    //初始化表格
    init_table();
    init_spec_table();
    init_info();
...
//根据权限初始化页面
function  inti_page() {
    if(hasAuthority('structure','auth_create')){
        $('#btn_add').removeClass('hide');
    }else{
        $('#btn_add').addClass('hide');
    }
    if(hasAuthority('structure','auth_delete')){
        ...
        columns:[//列数据
            ...
            },
            {
                ...
                formatter:function(value,row,index){//格式化，自定义内容
                    var _html='';
                    if(hasAuthority('structure','auth_edit')){
                        _html = '<button onclick="edit(\''+row.stId+'\')" class="btn btn-info btn-xs" data-toggle="tooltip" data-placement="bottom" title="修改"><i class="demo-psi-pen-5"></i></button>';
                    }
                    if(hasAuthority('structure','auth_delete')){
                    ...
                    if(hasAuthority('structure','auth_view')){
                        _html += '<button  onclick="check(\''+row.stId+'\')"class="btn btn-primary btn-xs" data-toggle="tooltip" data-placement="bottom" title="查看"><i class="fa fa-search"></i></button>'
                    }
                    return _html;
                },
            ...
            {
                field:'stWallThicknessUnitMm',//数据列
                title:'壁厚',//数据列名称
                sortable:true,//可排序
                align:'center',//水平居中
                valign:'middle',//垂直居中
            ...
            // { field:'stId',title:'stId',visible:false }//隐藏不显示
        ]
    });
}
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                ...
                var validateForm = $('#structureForm').data('bootstrapValidator');
                //手动触发验证
                ...
                var stId=$('#stId').val();
                var stStemDiameterUnitMm = $('#stStemDiameterUnitMm').val();
                var stWallThicknessUnitMm=$('#stWallThicknessUnitMm').val();
                var stMaximumInternodeLengthUnitCm=$('#stMaximumInternodeLengthUnitCm').val();
                var stRattanDiameterUnitMm = $('#stRattanDiameterUnitMm').val();
                ...
                if (stId == "") {//新增
                    formData.specId = 0;
                    $.ajax({
                        url: baseUrl + '/structure/save',		//请求路径
                        type: 'POST',			            //请求方式
...
function edit(id) {
    init_form();
    $.ajax({
        url:baseUrl+'/structure/findId/'+id,		//请求路径
        type:'GET',			                    //请求方式
...
function dele(gid){
    bootbox.confirm({
        ...
        callback: function(result) {
            if (result) {
                $.ajax({
                    url:baseUrl+'/structure/delete/'+gid,   //请求路径,单个删除
                    type:'DELETE',				        //请求方式
                    contentType: 'application/json',    //数据类型
                    success:function(res){	            //请求成功回调函数
                        if(res.code==200){
...
function init_form(){
    ...
    $('#specId').val("");
    $('#genusId').val("");
    $('#stStemDiameterUnitMm').val("");
    $('#stWallThicknessUnitMm').val("");
    $('#stMaximumInternodeLengthUnitCm').val("");
    $('#stRattanDiameterUnitMm').val("");
    $('#structureForm').data('bootstrapValidator').resetForm();
}
//初始化详情元素的值
function init_info(){
    $('#spec-check').val("").attr('data-original-title',"");//清除鼠标停留显示的内容，就是提示内容
    $('#stStemDiameterUnitMm-info').val("").attr('data-original-title',"");
    $('#stWallThicknessUnitMm-info').val("").attr('data-original-title',"");
    $('#stMaximumInternodeLengthUnitCm-info').val("").attr('data-original-title',"");
    $('#stRattanDiameterUnitMm-info').val("").attr('data-original-title',"");
}
//校验表单
function checkForm(){
    $("#structureForm").bootstrapValidator({
