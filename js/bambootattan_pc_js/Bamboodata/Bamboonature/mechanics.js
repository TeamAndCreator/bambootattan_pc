...
$(function(){
    queryPageUrl = baseUrl+'/mechanics/findAllQuery';
    //querySpecPageUrl = baseUrl+'/spec/findAllQuery';
    querySpecPageUrl = baseUrl+'/spec/findAllQueryWithOutMechanics';
    inti_page();
    //新增点击事件
...
//根据权限初始化页面
function  inti_page() {
    if(hasAuthority('mechanics','auth_create')){
        $('#btn_add').removeClass('hide');
    }else{
        $('#btn_add').addClass('hide');
    }
    if(hasAuthority('mechanics','auth_delete')){
        ...
        columns:[//列数据
            ...
            },
            {
                ...
                formatter:function(value,row,index){//格式化，自定义内容
                    var _html='';
                    if(hasAuthority('mechanics','auth_edit')){
                        _html = '<button onclick="edit(\''+row.mechId+'\')" class="btn btn-info btn-xs" data-toggle="tooltip" data-placement="bottom" title="修改"><i class="demo-psi-pen-5"></i></button>';
                    }
                    if(hasAuthority('mechanics','auth_delete')){
                        _html += '<button  onclick="dele(\''+row.mechId+'\')"class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="bottom" title="删除"><i class="demo-pli-cross"></i></button>';
                    }
                    if(hasAuthority('mechanics','auth_view')){
                        _html += '<button  onclick="check(\''+row.mechId+'\')"class="btn btn-primary btn-xs" data-toggle="tooltip" data-placement="bottom" title="查看"><i class="fa fa-search"></i></button>'
                    }
                    return _html;
                },
            ...
            {
                field:'mechComplianceUnitRouliang',//数据列
                title:'柔量',//数据列名称
                sortable:true,//可排序
                align:'center',//水平居中
                valign:'middle',//垂直居中
            ...
            // { field:'mechId',title:'mechId',visible:false }//隐藏不显示
        ]
    });
}
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                ...
                var validateForm = $('#mechanicsForm').data('bootstrapValidator');
                //手动触发验证
                ...
                var mechBendingModulusElastictyUnitGpa = $('#mechBendingModulusElastictyUnitGpa').val();
                var mechBendingStrenghtUnitMpa = $('#mechBendingStrenghtUnitMpa').val();
                ...
                if (mechId == "") {//新增
                    formData.specId = 0;
                    $.ajax({
                        url: baseUrl + '/mechanics/save',		//请求路径
                        type: 'POST',			            //请求方式
                    ...
                    $.ajax({
                        url: baseUrl + '/mechanics/update',	    //请求路径
                        type: 'PUT',				        //请求方式
                        data: JSON.stringify(formData),	    //数据
                        contentType: 'application/json',    //数据类型
                        success: function (res) {	        //请求成功回调函数
...
function edit(id) {
    init_form();
    $.ajax({
        url:baseUrl+'/mechanics/findId/'+id,		//请求路径
        type:'GET',			                    //请求方式
        ...
        success:function(res){	                //请求成功回调函数
            if(res.code==200){
                ...
                $('#mechShearStrenghtUnitMpa').val(res.data.mechShearStrenghtUnitMpa);
...
function dele(gid){
    bootbox.confirm({
        ...
        callback: function(result) {
            if (result) {
                $.ajax({
                    url:baseUrl+'/mechanics/delete/'+gid,   //请求路径,单个删除
                    type:'DELETE',				        //请求方式
                    contentType: 'application/json',    //数据类型
                    success:function(res){	            //请求成功回调函数
                        if(res.code==200){
...
function check(id) {
    ...
    $.ajax({
        ...
        success:function(res){	                //请求成功回调函数
            if(res.code==200){
                $('#mechBendingModulusElastictyUnitGpa-info').html(res.data.mechBendingModulusElastictyUnitGpa).attr('data-original-title',res.data.mechBendingModulusElastictyUnitGpa);
                $('#mechBendingStrenghtUnitMpa-info').html(res.data.mechBendingStrenghtUnitMpa).attr('data-original-title',res.data.mechBendingStrenghtUnitMpa);
                $('#mechCompressionStrenghtUnitMpa-info').html(res.data.mechCompressionStrenghtUnitMpa).attr('data-original-title',res.data.mechCompressionStrenghtUnitMpa);
                $('#mechShearStrenghtUnitMpa-info').html(res.data.mechShearStrenghtUnitMpa).attr('data-original-title',res.data.mechShearStrenghtUnitMpa);
            ...
            callback: function(result) {//点击按钮的回调事件，result:false-取消，true-确认
                if (result) {   //确认
                    ...
                    for(var i=0;i<selectedItems.length;i++){
                        ...
                        ids.push(selectedItems[i].mechId);
                    }
                    $.ajax({    //批量删除
                        //现将数据每个元素用‘,(逗号)’分隔拼接成字符串，再用encodeURI进行编码，最后拼接到url的后面
                        url: baseUrl+'/mechanics/deleteByIds?ids='+encodeURI(ids.join(',')),
                        type:'DELETE',
                        contentType: 'application/json',//数据类型
                        success:function(res){	        //请求成功回调函数
...
function init_form(){
    ...
    $('#specId').val("");
    $('#genusId').val("");
    $('#mechBendingModulusElastictyUnitGpa').val("");
    $('#mechBendingStrenghtUnitMpa').val("");
    $('#mechCompressionStrenghtUnitMpa').val("");
    $('#mechShearStrenghtUnitMpa').val("");
