...
$(function(){
    queryPageUrl = baseUrl+'/tissueproportion/findAllQuery';
    //querySpecPageUrl = baseUrl+'/spec/findAllQuery';
    querySpecPageUrl = baseUrl+'/spec/findAllQueryWithOutTissueproportion';
    inti_page();
    //新增点击事件
    ...
    $('#exampleModal').on('shown.bs.modal',function () {
        $('#tissueproportionForm').data('bootstrapValidator').resetForm();
    });
    //批量删除点击事件
    ...
    checkForm();
    //表单验证
    $('#tissueproportionForm').bootstrapValidator();
    //初始化表格
    init_table();
    init_spec_table();
    // //表单验证
...
//根据权限初始化页面
function  inti_page() {
    if(hasAuthority('tissueproportion','auth_create')){
        $('#btn_add').removeClass('hide');
    }else{
        $('#btn_add').addClass('hide');
    }
    if(hasAuthority('tissueproportion','auth_delete')){
        ...
        columns:[//列数据
            ...
            },
            {
                ...
                formatter:function(value,row,index){//格式化，自定义内容
                    var _html='';
                    if(hasAuthority('tissueproportion','auth_edit')){
                        _html = '<button onclick="edit(\''+row.tpId+'\')" class="btn btn-info btn-xs" data-toggle="tooltip" data-placement="bottom" title="修改"><i class="demo-psi-pen-5"></i></button>';
                    }
                    if(hasAuthority('tissueproportion','auth_delete')){
                    ...
                    if(hasAuthority('tissueproportion','auth_view')){
                        _html += '<button  onclick="check(\''+row.tpId+'\')"class="btn btn-primary btn-xs" data-toggle="tooltip" data-placement="bottom" title="查看"><i class="fa fa-search"></i></button>'
                    }
                    return _html;
                },
            ...
            {
                field:'tpFiberPeoportionUnitPercent',//数据列
                title:'纤维比量',//数据列名称
                sortable:true,//可排序
                align:'center',//水平居中
                valign:'middle',//垂直居中
            ...
            {
                field:'tpVesselProportionUnitPercent',//数据列
                title:'导管比量',//数据列名称
                sortable:true,//可排序
                align:'center',//水平居中
                valign:'middle',//垂直居中
            ...
            {
                field:'tpSieveTubeProportionUnitPercent',//数据列
                title:'筛管比量',//数据列名称
                sortable:true,//可排序
                align:'center',//水平居中
                valign:'middle',//垂直居中
            ...
            {
                field:'tpParenchymaCellProportionUnitPercent',//数据列
                title:'薄壁细胞比量',//数据列名称
                sortable:true,//可排序
                align:'center',//水平居中
                valign:'middle',//垂直居中
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                ...
                var validateForm = $('#tissueproportionForm').data('bootstrapValidator');
                //手动触发验证
                ...
                var tpParenchymaCellProportionUnitPercent=$('#tpParenchymaCellProportionUnitPercent').val();
                ...
                if (tpId == "") {//新增
                    ...
                    $.ajax({
                        url: baseUrl + '/tissueproportion/update',	    //请求路径
                        type: 'PUT',				        //请求方式
                        data: JSON.stringify(formData),	    //数据
                        contentType: 'application/json',    //数据类型
                        success: function (res) {	        //请求成功回调函数
...
function edit(id) {
    init_form();
    $.ajax({
        url:baseUrl+'/tissueproportion/findId/'+id,		//请求路径
        type:'GET',			                    //请求方式
...
function dele(gid){
    bootbox.confirm({
        ...
        callback: function(result) {
            if (result) {
                $.ajax({
                    url:baseUrl+'/tissueproportion/delete/'+gid,   //请求路径,单个删除
                    type:'DELETE',				        //请求方式
                    contentType: 'application/json',    //数据类型
                    success:function(res){	            //请求成功回调函数
                        if(res.code==200){
            ...
            callback: function(result) {//点击按钮的回调事件，result:false-取消，true-确认
                if (result) {   //确认
                    ...
                    $.ajax({    //批量删除
                        ...
                        url: baseUrl+'/tissueproportion/deleteByIds?ids='+encodeURI(ids.join(',')),
                        type:'DELETE',
                        contentType: 'application/json',//数据类型
                        success:function(res){	        //请求成功回调函数
...
function init_form(){
    ...
    $('#tpVesselProportionUnitPercent').val("");
    $('#tpSieveTubeProportionUnitPercent').val("");
    $('#tpParenchymaCellProportionUnitPercent').val("");
    $('#tissueproportionForm').data('bootstrapValidator').resetForm();
