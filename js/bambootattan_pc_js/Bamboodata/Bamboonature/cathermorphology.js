...
$(function(){
   ...
   // querySpecPageUrl = baseUrl+'/spec/findAllQuery';
    querySpecPageUrl = baseUrl+'/spec/findAllQueryWithOutCathermorphology';
    inti_page();
    //新增点击事件
    ...
    $('#exampleModal').on('shown.bs.modal',function () {
        $('#cathermorphologyForm').data('bootstrapValidator').resetForm();
    });
    //批量删除点击事件
    ...
    checkForm();
    //表单验证
    $('#cathermorphologyForm').bootstrapValidator();
    //初始化表格
    init_table();
    init_spec_table();
    // //表单验证
...
//根据权限初始化页面
function  inti_page() {
    if(hasAuthority('cathermorphology','auth_create')){
        $('#btn_add').removeClass('hide');
    }else{
        $('#btn_add').addClass('hide');
    }
    if(hasAuthority('cathermorphology','auth_delete')){
        ...
        columns:[//列数据
            ...
            },
            {
                ...
                formatter:function(value,row,index){//格式化，自定义内容
                    var _html='';
                    if(hasAuthority('cathermorphology','auth_edit')){
                        _html = '<button onclick="edit(\''+row.chmId+'\')" class="btn btn-info btn-xs" data-toggle="tooltip" data-placement="bottom" title="修改"><i class="demo-psi-pen-5"></i></button>';
                    }
                    if(hasAuthority('cathermorphology','auth_delete')){
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                ...
                var validateForm = $('#cathermorphologyForm').data('bootstrapValidator');
                //手动触发验证
