...
$(function(){
    ...
    $('#exampleModal').on('shown.bs.modal',function () {
        $('#vascularForm').data('bootstrapValidator').resetForm();
    });
    //批量删除点击事件
    ...
    checkForm();
    //表单验证
    $('#vascularForm').bootstrapValidator();
    //初始化表格
    init_table();
    init_spec_table();
    // //表单验证
...
//根据权限初始化页面
function  inti_page() {
    if(hasAuthority('rattanvascularbundelmorphology','auth_create')){
        $('#btn_add').removeClass('hide');
    }else{
        $('#btn_add').addClass('hide');
    }
    if(hasAuthority('rattanvascularbundelmorphology','auth_delete')){
        ...
        columns:[//列数据
            ...
            },
            {
                ...
                formatter:function(value,row,index){//格式化，自定义内容
                    var _html='';
                    if(hasAuthority('rattanvascularbundelmorphology','auth_edit')){
                        _html = '<button onclick="edit(\''+row.vbmId+'\')" class="btn btn-info btn-xs" data-toggle="tooltip" data-placement="bottom" title="修改"><i class="demo-psi-pen-5"></i></button>';
                    }
                    if(hasAuthority('rattanvascularbundelmorphology','auth_delete')){
                    ...
                    if(hasAuthority('rattanvascularbundelmorphology','auth_view')){
                        _html += '<button  onclick="check(\''+row.vbmId+'\')"class="btn btn-primary btn-xs" data-toggle="tooltip" data-placement="bottom" title="查看"><i class="fa fa-search"></i></button>'
                    }
                    return _html;
                },
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                ...
                var validateForm = $('#vascularForm').data('bootstrapValidator');
                //手动触发验证
                ...
                if (vbmId === "") {//新增
                    ...
                    $.ajax({
                        url: baseUrl + '/tVascularbundelmorphology/update',	    //请求路径
                        type: 'PUT',				        //请求方式
                        data: JSON.stringify(formData),	    //数据
                        contentType: 'application/json',    //数据类型
                        success: function (res) {	        //请求成功回调函数
...
function edit(id) {
    init_form();
    $.ajax({
        url:baseUrl+'/tVascularbundelmorphology/findId/'+id,		//请求路径
        type:'GET',			                    //请求方式
...
function dele(gid){
    bootbox.confirm({
        ...
        callback: function(result) {
            if (result) {
                $.ajax({
                    url:baseUrl+'/tVascularbundelmorphology/delete/'+gid,   //请求路径,单个删除
                    type:'DELETE',				        //请求方式
                    contentType: 'application/json',    //数据类型
                    success:function(res){	            //请求成功回调函数
                        if(res.code===200){
            ...
            callback: function(result) {//点击按钮的回调事件，result:false-取消，true-确认
                if (result) {   //确认
                    ...
                    $.ajax({    //批量删除
                        ...
                        url: baseUrl+'/tVascularbundelmorphology/deleteByIds?ids='+encodeURI(ids.join(',')),
                        type:'DELETE',
                        contentType: 'application/json',//数据类型
                        success:function(res){	        //请求成功回调函数
