...
$(function(){
    ...
    $('#exampleModal').on('shown.bs.modal',function () {
        $('#catherForm').data('bootstrapValidator').resetForm();
    });
    //批量删除点击事件
...
//根据权限初始化页面
function  inti_page() {
    if(hasAuthority('rattancathermorphology','auth_create')){
        $('#btn_add').removeClass('hide');
    }else{
        $('#btn_add').addClass('hide');
    }
    if(hasAuthority('rattancathermorphology','auth_delete')){
        ...
        columns:[//列数据
            ...
            },
            {
                ...
                formatter:function(value,row,index){//格式化，自定义内容
                    var _html='';
                    if(hasAuthority('rattancathermorphology','auth_edit')){
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                ...
                var validateForm = $('#catherForm').data('bootstrapValidator');
                //手动触发验证
