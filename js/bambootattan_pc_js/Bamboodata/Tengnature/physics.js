...
$(function(){
    ...
    $('#exampleModal').on('shown.bs.modal',function () {
        $('#phyForm').data('bootstrapValidator').resetForm();
    });
    //批量删除点击事件
...
//根据权限初始化页面
function  inti_page() {
    if(hasAuthority('rattanphysical','auth_create')){
        $('#btn_add').removeClass('hide');
    }else{
        $('#btn_add').addClass('hide');
    }
    if(hasAuthority('rattanphysical','auth_delete')){
        ...
        columns:[//列数据
            ...
            },
            {
                ...
                formatter:function(value,row,index){//格式化，自定义内容
                    var _html='';
                    if(hasAuthority('rattanphysical','auth_edit')){
                        _html = '<button onclick="edit(\''+row.phyId+'\')" class="btn btn-info btn-xs" data-toggle="tooltip" data-placement="bottom" title="修改"><i class="demo-psi-pen-5"></i></button>';
                    }
                    if(hasAuthority('rattanphysical','auth_delete')){
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                ...
                var validateForm = $('#phyForm').data('bootstrapValidator');
                //手动触发验证
                ...
                if (phyId === "") {//新增
                    ...
                    $.ajax({
                        url: baseUrl + '/tPhysics/update',	    //请求路径
                        type: 'PUT',				        //请求方式
...
function edit(id) {
    init_form();
    $.ajax({
        url:baseUrl+'/tPhysics/findId/'+id,		//请求路径
        type:'GET',			                    //请求方式
...
function dele(gid){
    bootbox.confirm({
        ...
        callback: function(result) {
            if (result) {
                $.ajax({
                    url:baseUrl+'/tPhysics/delete/'+gid,   //请求路径,单个删除
                    type:'DELETE',				        //请求方式
                    contentType: 'application/json',    //数据类型
                    success:function(res){	            //请求成功回调函数
                        if(res.code===200){
            ...
            callback: function(result) {//点击按钮的回调事件，result:false-取消，true-确认
                if (result) {   //确认
                    ...
                    $.ajax({    //批量删除
                        ...
                        url: baseUrl+'/tPhysics/deleteByIds?ids='+encodeURI(ids.join(',')),
                        type:'DELETE',
                        contentType: 'application/json',//数据类型
                        success:function(res){	        //请求成功回调函数
