...
$(function(){
    ...
    queryPageUrl = baseUrl+'/sheathear/findAllQuery';
    //querySpecPageUrl = baseUrl+'/spec/findAllQuery';
    querySpecPageUrl = baseUrl+'/spec/findAllQueryWithOutSheathear';
    inti_page();
    //新增点击事件
...
//根据权限初始化页面
function  inti_page() {
    if(hasAuthority('sheathear','auth_create')){
        $('#btn_add').removeClass('hide');
    }else{
        $('#btn_add').addClass('hide');
    }
    if(hasAuthority('sheathear','auth_delete')){
        ...
        columns:[//列数据
            ...
            },
            {
                ...
                formatter:function(value,row,index){//格式化，自定义内容
                    var _html='';
                    if(hasAuthority('sheathear','auth_edit')){
        ...
        callback: function (result) {
            if (result) {
                ...
                if (sheEarId == "") {//新增
                    formData.specId = 0;
                    $.ajax({
                        url: baseUrl + '/sheathear/save',		//请求路径
                        type: 'POST',			            //请求方式
                    ...
                    $.ajax({
                        url: baseUrl + '/sheathear/update',	    //请求路径
                        type: 'PUT',				        //请求方式
                        data: JSON.stringify(formData),	    //数据
                        contentType: 'application/json',    //数据类型
                        success: function (res) {	        //请求成功回调函数
