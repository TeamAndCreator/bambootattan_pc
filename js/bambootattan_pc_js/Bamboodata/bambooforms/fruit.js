...
$(function(){
    queryPageUrl = baseUrl+'/flowerfruit/findAllQuery';
    //querySpecPageUrl = baseUrl+'/spec/findAllQuery';
    querySpecPageUrl = baseUrl+'/spec/findAllQueryWithOutFlowerfruit';
    inti_page();
    //新增点击事件
    ...
    $('#exampleModal').on('shown.bs.modal',function () {
        $('#fruitForm').data('bootstrapValidator').resetForm();
    });
    //批量删除点击事件
...
//根据权限初始化页面
function  inti_page() {
    if(hasAuthority('fruit','auth_create')){
        $('#btn_add').removeClass('hide');
    }else{
        $('#btn_add').addClass('hide');
    }
    if(hasAuthority('fruit','auth_delete')){
        ...
        columns:[//列数据
            ...
            },
            {
                ...
                formatter:function(value,row,index){//格式化，自定义内容
                    var _html='';
                    if(hasAuthority('fruit','auth_edit')){
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                ...
                var floFruitId=$('#floFruitId').val();
                var spikeletShape = $('#spikeletShape').val();
                var spikeletBack = $('#spikeletBack').val();
                var spikeletFloret = $('#spikeletFloret').val();
                var stamenNum = $('#stamenNum').val();
                var glume=$('#glume').val();
                var lodicule = $('#lodicule').val();
                var palea = $('#palea').val();
                var lemma = $('#lemma').val();
                ...
                if (floFruitId == "") {//新增
                    formData.specId = 0;
                    $.ajax({
                        url: baseUrl + '/flowerfruit/save',		//请求路径
                        type: 'POST',			            //请求方式
                    ...
                    $.ajax({
                        url: baseUrl + '/flowerfruit/update',	    //请求路径
                        type: 'PUT',				        //请求方式
                        data: JSON.stringify(formData),	    //数据
                        contentType: 'application/json',    //数据类型
                        success: function (res) {	        //请求成功回调函数
...
function edit(id) {
    ...
    $.ajax({
        ...
        success:function(res){	                //请求成功回调函数
            if(res.code==200){
                $('#floFruitId').val(res.data.floFruitId);
                $('#spikeletShape').val(res.data.spikeletShape);
                $('#spikeletBack').val(res.data.spikeletBack);
                $('#spikeletFloret').val(res.data.spikeletFloret);
                $('#stamenNum').val(res.data.stamenNum);
                $('#glume').val(res.data.glume);
...
function dele(gid){
    bootbox.confirm({
        ...
        callback: function(result) {
            if (result) {
                $.ajax({
                    url:baseUrl+'/flowerfruit/delete/'+gid,   //请求路径,单个删除
                    type:'DELETE',				        //请求方式
                    contentType: 'application/json',    //数据类型
                    success:function(res){	            //请求成功回调函数
                        if(res.code==200){
            ...
            callback: function(result) {//点击按钮的回调事件，result:false-取消，true-确认
                if (result) {   //确认
                    ...
                    for(var i=0;i<selectedItems.length;i++){
                        ...
                        ids.push(selectedItems[i].floFruitId);
                    }
                    $.ajax({    //批量删除
                        //现将数据每个元素用‘,(逗号)’分隔拼接成字符串，再用encodeURI进行编码，最后拼接到url的后面
                        url: baseUrl+'/flowerfruit/deleteByIds?ids='+encodeURI(ids.join(',')),
                        type:'DELETE',
                        contentType: 'application/json',//数据类型
                        success:function(res){	        //请求成功回调函数
