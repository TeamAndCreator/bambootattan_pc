...
$(function(){
    queryPageUrl = baseUrl+'/tSheathtongue/findAllQuery';
    ...
    $('#exampleModal').on('shown.bs.modal',function () {
        $('#sheathtongueForm').data('bootstrapValidator').resetForm();
    });
    //批量删除点击事件
...
//根据权限初始化页面
function  inti_page() {
    if(hasAuthority('rattansheathtongue','auth_create')){
        $('#btn_add').removeClass('hide');
    }else{
        $('#btn_add').addClass('hide');
    }
    if(hasAuthority('rattansheathtongue','auth_delete')){
        ...
        columns:[//列数据
            ...
            },
            {
                ...
                formatter:function(value,row,index){//格式化，自定义内容
                    var _html='';
                    if(hasAuthority('rattansheathtongue','auth_edit')){
                        _html = '<button onclick="edit(\''+row.sheTogId+'\')" class="btn btn-info btn-xs" data-toggle="tooltip" data-placement="bottom" title="修改"><i class="demo-psi-pen-5"></i></button>';
                    }
                    if(hasAuthority('rattansheathtongue','auth_delete')){
                    ...
                    if(hasAuthority('rattansheathtongue','auth_view')){
                        _html += '<button  onclick="check(\''+row.sheTogId+'\')"class="btn btn-primary btn-xs" data-toggle="tooltip" data-placement="bottom" title="查看"><i class="fa fa-search"></i></button>'
                    }
                    return _html;
                },
        ...
        callback: function (result) {
            if (result) {
                ...
                if (sheTogId == "") {//新增
                    formData.specId = 0;
                    $.ajax({
                        url: baseUrl + '/tSheathtongue/save',		//请求路径
                        type: 'POST',			            //请求方式
...
function edit(id) {
    init_form();
    $.ajax({
        url:baseUrl+'/tSheathtongue/findId/'+id,		//请求路径
        type:'GET',			                    //请求方式
...
function dele(gid){
    bootbox.confirm({
        ...
        callback: function(result) {
            if (result) {
                $.ajax({
                    url:baseUrl+'/tSheathtongue/delete/'+gid,   //请求路径,单个删除
                    type:'DELETE',				        //请求方式
                    contentType: 'application/json',    //数据类型
                    success:function(res){	            //请求成功回调函数
                        if(res.code===200){
            ...
            callback: function(result) {//点击按钮的回调事件，result:false-取消，true-确认
                if (result) {   //确认
                    ...
                    $.ajax({    //批量删除
                        ...
                        url: baseUrl+'/tSheathtongue/deleteByIds?ids='+encodeURI(ids.join(',')),
                        type:'DELETE',
                        contentType: 'application/json',//数据类型
                        success:function(res){	        //请求成功回调函数
...
function check(id) {
    ...
    $.ajax({
        ...
        success:function(res){	                //请求成功回调函数
            if(res.code===200){
                $('#sheathTongueColor-info').html(res.data.sheathTongueColor).attr('data-original-title',res.data.sheathTongueColor);
                $('#sheathTongueHeight-info').html(res.data.sheathTongueHeight).attr('data-original-title',res.data.sheathTongueHeight);
...
function init_info(){
    ...
    $('#sheathTongueColor-info').val("").attr('data-original-title',"");
    $('#sheathTongueHeight-info').val("").attr('data-original-title',"");
    $('#sheathTongueMarginShape-info').val("").attr('data-original-title',"");
    $('#sheathTongueBackPowderv-info').val("").attr('data-original-title',"");
}
//校验表单
function checkForm(){
    $("#sheathtongueForm").bootstrapValidator({
