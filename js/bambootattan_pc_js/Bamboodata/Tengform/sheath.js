...
//根据权限初始化页面
function  inti_page() {
    if(hasAuthority('rattansheath','auth_create')){
        $('#btn_add').removeClass('hide');
    }else{
        $('#btn_add').addClass('hide');
    }
    if(hasAuthority('rattansheath','auth_delete')){
        ...
        columns:[//列数据
            ...
            },
            {
                ...
                formatter:function(value,row,index){//格式化，自定义内容
                    var _html='';
                    if(hasAuthority('rattansheath','auth_edit')){
                        _html = '<button onclick="edit(\''+row.sheId+'\')" class="btn btn-info btn-xs" data-toggle="tooltip" data-placement="bottom" title="修改"><i class="demo-psi-pen-5"></i></button>';
                    }
                    if(hasAuthority('rattansheath','auth_delete')){
    ...
    $.ajax({
        ...
        success:function(res){	                //请求成功回调函数
            if(res.code===200){
                ...
                //$('#genusId').val(res.data.rattanSpec.genus.genusId);
