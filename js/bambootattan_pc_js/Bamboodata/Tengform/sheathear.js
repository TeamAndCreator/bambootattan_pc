...
$(function(){
    queryPageUrl = baseUrl+'/tSheathear/findAllQuery';
    ...
    $('#exampleModal').on('shown.bs.modal',function () {
        $('#sheathearForm').data('bootstrapValidator').resetForm();
    });
    //批量删除点击事件
    ...
    checkForm();
    //表单验证
    $('#sheathearForm').bootstrapValidator();
    //初始化表格
    init_table();
    init_spec_table();
    // //表单验证
        ...
        columns:[//列数据
            ...
            },
            {
                ...
                formatter:function(value,row,index){//格式化，自定义内容
                    var _html='';
                    if(hasAuthority('rattansheathear','auth_edit')){
                        _html = '<button onclick="edit(\''+row.sheEarId+'\')" class="btn btn-info btn-xs" data-toggle="tooltip" data-placement="bottom" title="修改"><i class="demo-psi-pen-5"></i></button>';
                    }
                    if(hasAuthority('rattansheathear','auth_delete')){
                    ...
                    if(hasAuthority('rattansheathear','auth_view')){
                        _html += '<button  onclick="check(\''+row.sheEarId+'\')"class="btn btn-primary btn-xs" data-toggle="tooltip" data-placement="bottom" title="查看"><i class="fa fa-search"></i></button>'
                    }
                    return _html;
                },
            ...
            // { field:'sheEarId',title:'sheEarId',visible:false }//隐藏不显示
        ]
    });
}
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                ...
                var sheEarId=$('#sheEarId').val();
                var sheathEarDev = $('#sheathEarDev').val();
                var sheathEarShape = $('#sheathEarShape').val();
                var sheathEarMargin = $('#sheathEarMargin').val();
                ...
                if (sheEarId == "") {//新增
                    formData.specId = 0;
                    $.ajax({
                        url: baseUrl + '/tSheathear/save',		//请求路径
                        type: 'POST',			            //请求方式
                    ...
                    $.ajax({
                        url: baseUrl + '/tSheathear/update',	    //请求路径
                        type: 'PUT',				        //请求方式
                        data: JSON.stringify(formData),	    //数据
                        contentType: 'application/json',    //数据类型
                        success: function (res) {	        //请求成功回调函数
...
function edit(id) {
    init_form();
    $.ajax({
        url:baseUrl+'/tSheathear/findId/'+id,		//请求路径
        type:'GET',			                    //请求方式
        dataType:"JSON",		                //返回数据类型
        contentType: 'application/json',        //数据类型
        success:function(res){	                //请求成功回调函数
            if(res.code===200){
                $('#sheEarId').val(res.data.sheEarId);
...
function dele(gid){
    bootbox.confirm({
        ...
        callback: function(result) {
            if (result) {
                $.ajax({
                    url:baseUrl+'/tSheathear/delete/'+gid,   //请求路径,单个删除
                    type:'DELETE',				        //请求方式
                    contentType: 'application/json',    //数据类型
                    success:function(res){	            //请求成功回调函数
                        if(res.code===200){
            ...
            callback: function(result) {//点击按钮的回调事件，result:false-取消，true-确认
                if (result) {   //确认
                    ...
                    for(var i=0;i<selectedItems.length;i++){
                        ...
                        ids.push(selectedItems[i].sheEarId);
                    }
                    $.ajax({    //批量删除
                        //现将数据每个元素用‘,(逗号)’分隔拼接成字符串，再用encodeURI进行编码，最后拼接到url的后面
                        url: baseUrl+'/tSheathear/deleteByIds?ids='+encodeURI(ids.join(',')),
                        type:'DELETE',
                        contentType: 'application/json',//数据类型
                        success:function(res){	        //请求成功回调函数
