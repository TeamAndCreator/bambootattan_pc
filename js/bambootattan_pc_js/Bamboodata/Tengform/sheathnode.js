...
$(function(){
    queryPageUrl = baseUrl+'/tSheathnode/findAllQuery';
    ...
    $('#exampleModal').on('shown.bs.modal',function () {
        $('#sheathnodeForm').data('bootstrapValidator').resetForm();
    });
    //批量删除点击事件
    ...
    checkForm();
    //表单验证
    $('#sheathnodeForm').bootstrapValidator();
    //初始化表格
    init_table();
    init_spec_table();
    // //表单验证
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                ...
                var validateForm = $('#sheathnodeForm').data('bootstrapValidator');
                //手动触发验证
                ...
                if (sheNodeId === "") {//新增
                    formData.specId = 0;
                    $.ajax({
                        url: baseUrl + '/tSheathnode/save',		//请求路径
                        type: 'POST',			            //请求方式
                    ...
                    $.ajax({
                        url: baseUrl + '/tSheathnode/update',	    //请求路径
                        type: 'PUT',				        //请求方式
                        data: JSON.stringify(formData),	    //数据
                        contentType: 'application/json',    //数据类型
                        success: function (res) {	        //请求成功回调函数
...
function edit(id) {
    init_form();
    $.ajax({
        url:baseUrl+'/tSheathnode/findId/'+id,		//请求路径
        type:'GET',			                    //请求方式
...
function dele(gid){
    bootbox.confirm({
        ...
        callback: function(result) {
            if (result) {
                $.ajax({
                    url:baseUrl+'/tSheathnode/delete/'+gid,   //请求路径,单个删除
                    type:'DELETE',				        //请求方式
                    contentType: 'application/json',    //数据类型
                    success:function(res){	            //请求成功回调函数
                        if(res.code===200){
            ...
            callback: function(result) {//点击按钮的回调事件，result:false-取消，true-确认
                if (result) {   //确认
                    ...
                    $.ajax({    //批量删除
                        ...
                        url: baseUrl+'/tSheathnode/deleteByIds?ids='+encodeURI(ids.join(',')),
                        type:'DELETE',
                        contentType: 'application/json',//数据类型
                        success:function(res){	        //请求成功回调函数
...
function check(id) {
    ...
    $.ajax({
        ...
        success:function(res){	                //请求成功回调函数
            if(res.code===200){
                $('#sheathNode-info').html(res.data.sheathNode).attr('data-original-title',res.data.sheathNode);
                $('#sheathNodeBack-info').html(res.data.sheathNodeBack).attr('data-original-title',res.data.sheathNodeBack);
...
//初始化详情元素的值
function init_info(){
    $('#sheathNode-info').val("").attr('data-original-title',"");//清除鼠标停留显示的内容，就是提示内容
    $('#sheathNodeBack-info').val("").attr('data-original-title',"");
}
//校验表单
function checkForm(){
    $("#sheathnodeForm").bootstrapValidator({
